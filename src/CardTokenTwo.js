...
function Inflation() {
  ...
  function Token() {
    try {
      coinName = geodeInfo.chain.toString().substring(0,5)
...
function TotalTreasury() {
  let freeBalance = 0 
  try {
    freeBalance = Math.round(geodeInfo.balance.data.free.toString()/microToCoin)
    //geodeInfo.balance.data.free.toHuman() //.substring(0,5);
  } catch(e) {
    console.error(e)
    freeBalance = 0;
  }
